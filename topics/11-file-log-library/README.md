...
실무에서 자주 만나는 I/O와 상태 전이를 연습할 수 있다.
...
- 이 토픽의 fresh template은 `make run`에서 `hello log`를 출력합니다.
...
- 템플릿 기본 상태: `make -C cpp run`은 `hello log`를 출력합니다.
...
- 직접 실행 예시:
  - `make -C topics/11-file-log-library/cpp build`
  - `topics/11-file-log-library/cpp/bin/t11_file_log_library_cpp topics/11-file-log-library/fixtures/events.txt`
