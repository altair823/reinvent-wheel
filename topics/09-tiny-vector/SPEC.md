# SPEC: 09. 작은 벡터/문자열 클래스 구현

## 템플릿 baseline

- fresh template의 `make run` 출력은 `hello vector`입니다.
- 이 baseline은 시작점일 뿐이며, 아래 요구사항 전체를 만족하지는 않습니다.
- `make smoke`는 baseline 실행 가능 여부를 확인하고, `make e2e`는 아래 미션 성공 기준을 검증합니다.

## 요구사항

1. 프로그램은 `tiny-vector` 토픽의 mission fixture를 처리하고 결정적인 결과를 출력해야 합니다.
...
6. push/pop과 capacity 증가가 동작해야 합니다.
7. 복사/이동 의미론을 깨지 않는 테스트를 포함해야 합니다.
...
- iterator; small buffer optimization; allocator 분리
