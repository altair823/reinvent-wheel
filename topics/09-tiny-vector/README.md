...
- 이 토픽의 fresh template은 `make run`에서 `hello vector`를 출력합니다.
...
- 템플릿 기본 상태: `make -C cpp run`은 `hello vector`를 출력합니다.
- CLI 스캐폴드: 템플릿 진입점이 `argv` 위치 계약만 이미 고정해 두므로, 보통은 라이브러리/코어 로직 구현에만 집중하면 됩니다.
- 인자 위치 계약:
  - `argv[1]`: `input` (`예: ../fixtures/values.txt`)
- Make 명령: `make -C cpp build`, `make -C cpp run`, `make -C cpp test`, `make -C cpp e2e`, `make -C cpp smoke`
- 직접 실행 예시:
  - `make -C topics/09-tiny-vector/cpp build`
  - `topics/09-tiny-vector/cpp/bin/t09_tiny_vector_cpp topics/09-tiny-vector/fixtures/values.txt`
