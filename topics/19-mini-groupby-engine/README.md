...
데이터 엔지니어링 업무의 가장 빈번한 흐름을 직접 만든다.
...
- 이 토픽의 fresh template은 `make run`에서 `hello groupby`를 출력합니다.
...
- 템플릿 기본 상태: `make -C rust run`은 `hello groupby`를 출력합니다.
...
- 핵심 로직: `python/src/t19_mini_groupby_engine_python/core.py`
- CLI 진입점: `python/src/t19_mini_groupby_engine_python/cli.py`
...
- 템플릿 기본 상태: `make -C python run`은 `hello groupby`를 출력합니다.
- CLI 스캐폴드: 템플릿 진입점이 `argv` 위치 계약만 이미 고정해 두므로, 보통은 라이브러리/코어 로직 구현에만 집중하면 됩니다.
- 인자 위치 계약:
  - `argv[1]`: `input` (`예: ../fixtures/sales.csv`)
- Make 명령: `make -C python run`, `make -C python test`, `make -C python e2e`, `make -C python smoke`
- 직접 실행 예시:
  - `source scripts/env.sh && PYTHONPATH=topics/19-mini-groupby-engine/python/src "$PYTHON_BIN" -m t19_mini_groupby_engine_python.cli topics/19-mini-groupby-engine/fixtures/sales.csv`
