# SPEC: 07. `arena allocator`

## 템플릿 baseline

- fresh template의 `make run` 출력은 `hello arena`입니다.
- 이 baseline은 시작점일 뿐이며, 아래 요구사항 전체를 만족하지는 않습니다.
- `make smoke`는 baseline 실행 가능 여부를 확인하고, `make e2e`는 아래 미션 성공 기준을 검증합니다.

## 요구사항

1. 프로그램은 `arena-allocator` 토픽의 mission fixture를 처리하고 결정적인 결과를 출력해야 합니다.
...
6. arena는 문자열 slice 또는 index 기반 핸들을 돌려줘야 합니다.
7. clear 후 재사용 가능해야 합니다.
...
- typed arena; bump allocator; unsafe 최적화
