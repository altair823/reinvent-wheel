...
- 이 토픽의 fresh template은 `make run`에서 `hello log-analyzer`를 출력합니다.
...
- 템플릿 기본 상태: `make -C rust run`은 `hello log-analyzer`를 출력합니다.
...
- 직접 실행 예시:
  - `source scripts/env.sh && cargo build --manifest-path topics/21-parallel-log-analyzer/rust/Cargo.toml`
  - `source scripts/env.sh && cargo run --manifest-path topics/21-parallel-log-analyzer/rust/Cargo.toml -- topics/21-parallel-log-analyzer/fixtures/app.log 2`
  - `./target/debug/t21_parallel_log_analyzer_rust topics/21-parallel-log-analyzer/fixtures/app.log 2`
- e2e 스크립트: `e2e/rust-smoke.sh`

### Python

- 핵심 로직: `python/src/t21_parallel_log_analyzer_python/core.py`
- CLI 진입점: `python/src/t21_parallel_log_analyzer_python/cli.py`
...
- 템플릿 기본 상태: `make -C python run`은 `hello log-analyzer`를 출력합니다.
- CLI 스캐폴드: 템플릿 진입점이 `argv` 위치 계약만 이미 고정해 두므로, 보통은 라이브러리/코어 로직 구현에만 집중하면 됩니다.
- 인자 위치 계약:
  - `argv[1]`: `input` (`예: ../fixtures/app.log`)
  - `argv[2]`: 선택 사항 `workers` (`기본값: 2`)
