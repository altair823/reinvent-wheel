...
make -s --no-print-directory run > /tmp/parallel-log-analyzer-python-out.txt
if ! diff -u "$ROOT/fixtures/mission-expected.txt" /tmp/parallel-log-analyzer-python-out.txt; then
