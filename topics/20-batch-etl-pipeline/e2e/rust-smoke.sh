...
make -s --no-print-directory run > /tmp/batch-etl-pipeline-rust-out.txt
if ! diff -u "$ROOT/fixtures/mission-expected.txt" /tmp/batch-etl-pipeline-rust-out.txt; then
