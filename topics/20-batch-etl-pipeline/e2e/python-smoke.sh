...
make -s --no-print-directory run > /tmp/batch-etl-pipeline-python-out.txt
if ! diff -u "$ROOT/fixtures/mission-expected.txt" /tmp/batch-etl-pipeline-python-out.txt; then
