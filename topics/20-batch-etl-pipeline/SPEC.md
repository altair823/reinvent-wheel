# SPEC: 20. batch ETL pipeline

## 템플릿 baseline

- fresh template의 `make run` 출력은 `hello etl`입니다.
- 이 baseline은 시작점일 뿐이며, 아래 요구사항 전체를 만족하지는 않습니다.
- `make smoke`는 baseline 실행 가능 여부를 확인하고, `make e2e`는 아래 미션 성공 기준을 검증합니다.

## 요구사항

1. 프로그램은 `batch-etl-pipeline` 토픽의 mission fixture를 처리하고 결정적인 결과를 출력해야 합니다.
...
6. 잘못된 row는 reject로 분리해야 합니다.
7. cleaned 수와 rejected 수를 함께 보고해야 합니다.
