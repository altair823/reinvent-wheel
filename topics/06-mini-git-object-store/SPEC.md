# SPEC: 06. 미니 `git` 객체 저장소

## 템플릿 baseline

- fresh template의 `make run` 출력은 `hello git-object`입니다.
- 이 baseline은 시작점일 뿐이며, 아래 요구사항 전체를 만족하지는 않습니다.
- `make smoke`는 baseline 실행 가능 여부를 확인하고, `make e2e`는 아래 미션 성공 기준을 검증합니다.

## 요구사항

1. 프로그램은 `mini-git-object-store` 토픽의 mission fixture를 처리하고 결정적인 결과를 출력해야 합니다.
...
6. blob 파일을 읽고 `.git-lite/objects` 아래에 저장해야 합니다.
7. 저장 후 객체 id와 원문 복원을 확인할 수 있어야 합니다.
...
- tree 비슷한 구조; commit 객체; 간단한 참조 관리
