...
- 이 토픽의 fresh template은 `make run`에서 `hello git-object`를 출력합니다.
...
- 템플릿 기본 상태: `make -C rust run`은 `hello git-object`를 출력합니다.
- CLI 스캐폴드: 템플릿 진입점이 `argv` 위치 계약만 이미 고정해 두므로, 보통은 라이브러리/코어 로직 구현에만 집중하면 됩니다.
- 인자 위치 계약:
  - `argv[1]`: `input` (`예: ../fixtures/blob.txt`)
- Make 명령: `make -C rust build`, `make -C rust run`, `make -C rust test`, `make -C rust e2e`, `make -C rust smoke`
- 직접 실행 예시:
  - `source scripts/env.sh && cargo build --manifest-path topics/06-mini-git-object-store/rust/Cargo.toml`
  - `source scripts/env.sh && cargo run --manifest-path topics/06-mini-git-object-store/rust/Cargo.toml -- topics/06-mini-git-object-store/fixtures/blob.txt`
  - `./target/debug/t06_mini_git_object_store_rust topics/06-mini-git-object-store/fixtures/blob.txt`
