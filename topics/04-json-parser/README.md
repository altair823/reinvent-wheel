...
- 이 토픽의 fresh template은 `make run`에서 `hello json`를 출력합니다.
...
- 템플릿 기본 상태: `make -C rust run`은 `hello json`를 출력합니다.
...
- 직접 실행 예시:
  - `source scripts/env.sh && cargo build --manifest-path topics/04-json-parser/rust/Cargo.toml`
  - `source scripts/env.sh && cargo run --manifest-path topics/04-json-parser/rust/Cargo.toml -- topics/04-json-parser/fixtures/sample.json`
  - `./target/debug/t04_json_parser_rust topics/04-json-parser/fixtures/sample.json`
...
- 템플릿 기본 상태: `make -C cpp run`은 `hello json`를 출력합니다.
- CLI 스캐폴드: 템플릿 진입점이 `argv` 위치 계약만 이미 고정해 두므로, 보통은 라이브러리/코어 로직 구현에만 집중하면 됩니다.
- 인자 위치 계약:
  - `argv[1]`: `input` (`예: ../fixtures/sample.json`)
...
- 핵심 로직/앱: `java/src/main/java/dev/reinvent/wheel/t04/JsonParserApp.java`
- 기본 테스트: `java/src/test/java/dev/reinvent/wheel/t04/JsonParserAppTest.java`
...
- 템플릿 기본 상태: `make -C java run`은 `hello json`를 출력합니다.
- CLI 스캐폴드: 템플릿 진입점이 `argv` 위치 계약만 이미 고정해 두므로, 보통은 라이브러리/코어 로직 구현에만 집중하면 됩니다.
- 인자 위치 계약:
  - `argv[1]`: `input` (`예: ../fixtures/sample.json`)
