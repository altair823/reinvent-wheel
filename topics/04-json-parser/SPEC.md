# SPEC: 04. 간단한 `JSON parser`

## 템플릿 baseline

- fresh template의 `make run` 출력은 `hello json`입니다.
- 이 baseline은 시작점일 뿐이며, 아래 요구사항 전체를 만족하지는 않습니다.
- `make smoke`는 baseline 실행 가능 여부를 확인하고, `make e2e`는 아래 미션 성공 기준을 검증합니다.

## 요구사항

1. 프로그램은 `json-parser` 토픽의 mission fixture를 처리하고 결정적인 결과를 출력해야 합니다.
...
6. 객체와 배열을 포함한 기본 JSON 값 파싱이 가능해야 합니다.
7. top-level key 순서는 fixture 기준으로 안정적이어야 합니다.
...
- pretty printer; JSON path 조회; streaming parser
