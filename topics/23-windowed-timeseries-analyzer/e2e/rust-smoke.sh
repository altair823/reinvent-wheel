...
make -s --no-print-directory run > /tmp/windowed-timeseries-analyzer-rust-out.txt
if ! diff -u "$ROOT/fixtures/mission-expected.txt" /tmp/windowed-timeseries-analyzer-rust-out.txt; then
