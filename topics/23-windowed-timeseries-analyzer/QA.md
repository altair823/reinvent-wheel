# QA: 23. windowed timeseries analyzer

## Template Baseline

- `make run`이 `hello timeseries`를 출력해야 한다.
...
- SQL 기준선과 Rust/Python 결과를 비교한다.

## Mission Success

- `e2e/*.sh`가 `fixtures/mission-expected.txt`와 실제 출력을 비교한다.
- 성공 시 Rust/Python 출력이 `sql/query.sql` 기준선과도 모순되지 않아야 한다.
