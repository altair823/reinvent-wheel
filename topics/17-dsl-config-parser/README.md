...
- 이 토픽의 fresh template은 `make run`에서 `hello dsl`를 출력합니다.
...
- 핵심 로직/앱: `kotlin/src/main/kotlin/dev/reinvent/wheel/t17/DslConfigParserApp.kt`
- 기본 테스트: `kotlin/src/test/kotlin/dev/reinvent/wheel/t17/DslConfigParserAppTest.kt`
...
- 템플릿 기본 상태: `make -C kotlin run`은 `hello dsl`를 출력합니다.
- CLI 스캐폴드: 템플릿 진입점이 `argv` 위치 계약만 이미 고정해 두므로, 보통은 라이브러리/코어 로직 구현에만 집중하면 됩니다.
- 인자 위치 계약:
  - `argv[1]`: `input` (`예: ../fixtures/sample.dsl`)
- Make 명령: `make -C kotlin build`, `make -C kotlin run`, `make -C kotlin test`, `make -C kotlin e2e`, `make -C kotlin smoke`
- 직접 실행 예시:
  - `source scripts/env.sh && ./gradlew :t17-dsl-config-parser-kotlin:installDist`
  - `source scripts/env.sh && ./gradlew -q :t17-dsl-config-parser-kotlin:run --args='topics/17-dsl-config-parser/fixtures/sample.dsl'`
  - `source scripts/env.sh && topics/17-dsl-config-parser/kotlin/build/install/t17-dsl-config-parser-kotlin/bin/t17-dsl-config-parser-kotlin topics/17-dsl-config-parser/fixtures/sample.dsl`
