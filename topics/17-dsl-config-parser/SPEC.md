# SPEC: 17. DSL 스타일 설정 파서

## 템플릿 baseline

- fresh template의 `make run` 출력은 `hello dsl`입니다.
- 이 baseline은 시작점일 뿐이며, 아래 요구사항 전체를 만족하지는 않습니다.
- `make smoke`는 baseline 실행 가능 여부를 확인하고, `make e2e`는 아래 미션 성공 기준을 검증합니다.

## 요구사항

1. 프로그램은 `dsl-config-parser` 토픽의 mission fixture를 처리하고 결정적인 결과를 출력해야 합니다.
...
6. 블록과 리스트 문법을 파싱해 정규화된 키/값으로 출력해야 합니다.
7. 문법 오류는 위치 정보를 포함해 실패해야 합니다.
