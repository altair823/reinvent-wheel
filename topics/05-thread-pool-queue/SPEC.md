...
- fresh template의 `make run` 출력은 `hello thread-pool`입니다.
- 이 baseline은 시작점일 뿐이며, 아래 요구사항 전체를 만족하지는 않습니다.
- `make smoke`는 baseline 실행 가능 여부를 확인하고, `make e2e`는 아래 미션 성공 기준을 검증합니다.

## 요구사항

1. 프로그램은 `thread-pool-queue` 토픽의 mission fixture를 처리하고 결정적인 결과를 출력해야 합니다.
...
6. 고정된 worker 수로 작업을 실행해야 합니다.
7. 결과 출력 순서는 입력 순서를 유지해야 합니다.
...
- future/promise 스타일 결과 반환; 우선순위 큐; work stealing 흉내내기
