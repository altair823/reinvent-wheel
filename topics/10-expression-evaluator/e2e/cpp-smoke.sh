...
make -s --no-print-directory run > /tmp/expression-evaluator-cpp-out.txt
if ! diff -u "$ROOT/fixtures/mission-expected.txt" /tmp/expression-evaluator-cpp-out.txt; then
