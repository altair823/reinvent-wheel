# 02. 인메모리 `key-value store`

작은 시스템 설계와 상태 전이를 연습하기 좋다.
...
- 이 토픽의 fresh template은 `make run`에서 `hello kv-store`를 출력합니다.
...
- 템플릿 기본 상태: `make -C rust run`은 `hello kv-store`를 출력합니다.
- CLI 스캐폴드: 템플릿 진입점이 `argv` 위치 계약만 이미 고정해 두므로, 보통은 라이브러리/코어 로직 구현에만 집중하면 됩니다.
- 인자 위치 계약:
  - `argv[1]`: `input` (`예: ../fixtures/commands.txt`)
- Make 명령: `make -C rust build`, `make -C rust run`, `make -C rust test`, `make -C rust e2e`, `make -C rust smoke`
- 직접 실행 예시:
  - `source scripts/env.sh && cargo build --manifest-path topics/02-key-value-store/rust/Cargo.toml`
  - `source scripts/env.sh && cargo run --manifest-path topics/02-key-value-store/rust/Cargo.toml -- topics/02-key-value-store/fixtures/commands.txt`
  - `./target/debug/t02_key_value_store_rust topics/02-key-value-store/fixtures/commands.txt`
- e2e 스크립트: `e2e/rust-smoke.sh`

### Java

- 핵심 로직/앱: `java/src/main/java/dev/reinvent/wheel/t02/KeyValueStoreApp.java`
- 기본 테스트: `java/src/test/java/dev/reinvent/wheel/t02/KeyValueStoreAppTest.java`
...
- 템플릿 기본 상태: `make -C java run`은 `hello kv-store`를 출력합니다.
- CLI 스캐폴드: 템플릿 진입점이 `argv` 위치 계약만 이미 고정해 두므로, 보통은 라이브러리/코어 로직 구현에만 집중하면 됩니다.
- 인자 위치 계약:
  - `argv[1]`: `input` (`예: ../fixtures/commands.txt`)
- Make 명령: `make -C java build`, `make -C java run`, `make -C java test`, `make -C java e2e`, `make -C java smoke`
- 직접 실행 예시:
  - `source scripts/env.sh && ./gradlew :t02-key-value-store-java:installDist`
  - `source scripts/env.sh && ./gradlew -q :t02-key-value-store-java:run --args='topics/02-key-value-store/fixtures/commands.txt'`
  - `source scripts/env.sh && topics/02-key-value-store/java/build/install/t02-key-value-store-java/bin/t02-key-value-store-java topics/02-key-value-store/fixtures/commands.txt`
- e2e 스크립트: `e2e/java-smoke.sh`

### Kotlin

- 핵심 로직/앱: `kotlin/src/main/kotlin/dev/reinvent/wheel/t02/KeyValueStoreApp.kt`
- 기본 테스트: `kotlin/src/test/kotlin/dev/reinvent/wheel/t02/KeyValueStoreAppTest.kt`
...
- 템플릿 기본 상태: `make -C kotlin run`은 `hello kv-store`를 출력합니다.
- CLI 스캐폴드: 템플릿 진입점이 `argv` 위치 계약만 이미 고정해 두므로, 보통은 라이브러리/코어 로직 구현에만 집중하면 됩니다.
- 인자 위치 계약:
  - `argv[1]`: `input` (`예: ../fixtures/commands.txt`)
- Make 명령: `make -C kotlin build`, `make -C kotlin run`, `make -C kotlin test`, `make -C kotlin e2e`, `make -C kotlin smoke`
- 직접 실행 예시:
  - `source scripts/env.sh && ./gradlew :t02-key-value-store-kotlin:installDist`
  - `source scripts/env.sh && ./gradlew -q :t02-key-value-store-kotlin:run --args='topics/02-key-value-store/fixtures/commands.txt'`
  - `source scripts/env.sh && topics/02-key-value-store/kotlin/build/install/t02-key-value-store-kotlin/bin/t02-key-value-store-kotlin topics/02-key-value-store/fixtures/commands.txt`
