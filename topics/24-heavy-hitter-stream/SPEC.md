# SPEC: 24. heavy hitter stream

## 템플릿 baseline

- fresh template의 `make run` 출력은 `hello heavy-hitter`입니다.
- 이 baseline은 시작점일 뿐이며, 아래 요구사항 전체를 만족하지는 않습니다.
- `make smoke`는 baseline 실행 가능 여부를 확인하고, `make e2e`는 아래 미션 성공 기준을 검증합니다.

## 요구사항

1. 프로그램은 `heavy-hitter-stream` 토픽의 mission fixture를 처리하고 결정적인 결과를 출력해야 합니다.
...
6. exact top-N과 approximate top-N을 모두 출력해야 합니다.
7. approx 결과에는 최소 exact 상위 key가 포함돼야 합니다.
...
- Count-Min Sketch; 메모리 상한; 슬라이딩 윈도우
