...
- 이 토픽의 fresh template은 `make run`에서 `hello heavy-hitter`를 출력합니다.
...
- 템플릿 기본 상태: `make -C rust run`은 `hello heavy-hitter`를 출력합니다.
- CLI 스캐폴드: 템플릿 진입점이 `argv` 위치 계약만 이미 고정해 두므로, 보통은 라이브러리/코어 로직 구현에만 집중하면 됩니다.
- 인자 위치 계약:
  - `argv[1]`: `input` (`예: ../fixtures/events.txt`)
- Make 명령: `make -C rust build`, `make -C rust run`, `make -C rust test`, `make -C rust e2e`, `make -C rust smoke`
- 직접 실행 예시:
  - `source scripts/env.sh && cargo build --manifest-path topics/24-heavy-hitter-stream/rust/Cargo.toml`
  - `source scripts/env.sh && cargo run --manifest-path topics/24-heavy-hitter-stream/rust/Cargo.toml -- topics/24-heavy-hitter-stream/fixtures/events.txt`
  - `./target/debug/t24_heavy_hitter_stream_rust topics/24-heavy-hitter-stream/fixtures/events.txt`
- e2e 스크립트: `e2e/rust-smoke.sh`

### Python

- 핵심 로직: `python/src/t24_heavy_hitter_stream_python/core.py`
- CLI 진입점: `python/src/t24_heavy_hitter_stream_python/cli.py`
...
- 템플릿 기본 상태: `make -C python run`은 `hello heavy-hitter`를 출력합니다.
- CLI 스캐폴드: 템플릿 진입점이 `argv` 위치 계약만 이미 고정해 두므로, 보통은 라이브러리/코어 로직 구현에만 집중하면 됩니다.
- 인자 위치 계약:
  - `argv[1]`: `input` (`예: ../fixtures/events.txt`)
- Make 명령: `make -C python run`, `make -C python test`, `make -C python e2e`, `make -C python smoke`
- 직접 실행 예시:
  - `source scripts/env.sh && PYTHONPATH=topics/24-heavy-hitter-stream/python/src "$PYTHON_BIN" -m t24_heavy_hitter_stream_python.cli topics/24-heavy-hitter-stream/fixtures/events.txt`
