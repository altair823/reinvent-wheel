...
- 이 토픽의 fresh template은 `make run`에서 `hello rate-limiter`를 출력합니다.
...
- 핵심 로직/앱: `java/src/main/java/dev/reinvent/wheel/t14/RateLimiterApp.java`
- 기본 테스트: `java/src/test/java/dev/reinvent/wheel/t14/RateLimiterAppTest.java`
...
- 템플릿 기본 상태: `make -C java run`은 `hello rate-limiter`를 출력합니다.
- CLI 스캐폴드: 템플릿 진입점이 `argv` 위치 계약만 이미 고정해 두므로, 보통은 라이브러리/코어 로직 구현에만 집중하면 됩니다.
- 인자 위치 계약:
  - `argv[1]`: `input` (`예: ../fixtures/requests.txt`)
- Make 명령: `make -C java build`, `make -C java run`, `make -C java test`, `make -C java e2e`, `make -C java smoke`
- 직접 실행 예시:
  - `source scripts/env.sh && ./gradlew :t14-rate-limiter-java:installDist`
  - `source scripts/env.sh && ./gradlew -q :t14-rate-limiter-java:run --args='topics/14-rate-limiter/fixtures/requests.txt'`
  - `source scripts/env.sh && topics/14-rate-limiter/java/build/install/t14-rate-limiter-java/bin/t14-rate-limiter-java topics/14-rate-limiter/fixtures/requests.txt`
