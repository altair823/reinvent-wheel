# 08. `tiny async executor`

async의 내부 모델을 손으로 이해하기 좋다.
...
- 이 토픽의 fresh template은 `make run`에서 `hello async`를 출력합니다.
...
- 템플릿 기본 상태: `make -C rust run`은 `hello async`를 출력합니다.
...
- 직접 실행 예시:
  - `source scripts/env.sh && cargo build --manifest-path topics/08-tiny-async-executor/rust/Cargo.toml`
  - `source scripts/env.sh && cargo run --manifest-path topics/08-tiny-async-executor/rust/Cargo.toml -- topics/08-tiny-async-executor/fixtures/tasks.txt`
  - `./target/debug/t08_tiny_async_executor_rust topics/08-tiny-async-executor/fixtures/tasks.txt`
